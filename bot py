...
def check_rate_limit(chat_id: int) -> bool:
    now = time.monotonic()
    if now - rate_limit_cache[chat_id] < RATE_LIMIT_SECS:
        return False
...
async def cmd_setup(message):
    ...
    if await check_session_url(url):
        user_data[chat_id] = {'session_url': url, 'last_active': time.monotonic()}
...
async def cleanup_old_data():
    while not _shutdown_event.is_set():
        ...
        now = time.monotonic()
        stale = [k for k, v in rate_limit_cache.items() if now - v > 3600]
        for k in stale:
            del rate_limit_cache[k]
        logger.info(f"Cleaned {len(stale)} stale rate limit entries")
